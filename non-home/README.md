...
But, this is just for organizational purposes. See `deployments.yaml` for the
actual destination paths used in deployment.

## Deployment manifest

`non-home.yaml` is the authoritative list of what gets deployed and where. Each
entry declares:
...
- `instructions` — text shown after the file is updated, prompting any manual
  follow-up steps

The deployment is driven by `home/.chezmoiscripts/after/run_after_script.py`,
which reads this manifest and deploys each applicable entry.
