...
function fish_prompt --description "Write out my prompt"
    ...
    printf '\n%s@%s %s %s\n%s ↳%s %s ' \
        (_colorize $USER $d3_pink) \
        (_colorize $hostname $d3_pink) \
        (date --iso-8601=seconds) \
        (fish_git_prompt; or echo "") \
        (_colorize $PWD --underline $d3_blue) \
        (_colorize $_status (test $_status -eq 0; and echo $d3_green; or echo $d3_red)) \
        (fish_is_root_user; and _colorize "#" $d3_red; or _colorize "%" $d3_green)
end
