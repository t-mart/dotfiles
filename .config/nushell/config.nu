...
def yt-beet-import [
  ...
  mkdir $download_dir
  yt-dlp --output $"($download_dir)/%\(track_number,playlist_index)02d - %\(artist)s - %\(album,playlist)s - %\(track)s.%\(ext)s" --concurrent-fragments 3 --extract-audio --format "bestaudio*[acodec=opus]/bestaudio*" $playlist_url
